...
export interface AiProvider {
  ...
  // toolsUsed is surfaced to the UI so the caller can show which internal tools actually backed the answer.
  converse?(input: { instructions: string; question: string; history: { question: string; answer: string }[]; tools: AiToolSpec[]; call: AiToolCall; signal?:AbortSignal }): Promise<{ text: string; toolsUsed: string[] }>;
}
