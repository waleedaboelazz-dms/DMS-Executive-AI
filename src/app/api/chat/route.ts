...
export async function POST(request: Request) { try {
  ...
  const answer = await new ExecutiveAgent(new OpenAiProvider()).chat({dataset,question:input.question,language:input.language,history:history.reverse().map(h=>({question:h.question,answer:h.answer}))});
